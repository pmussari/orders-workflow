...
export class ActionService {
  ...
  private buildContext(chatId: string): string {
    const orders = this.repo.findByChat(chatId);
    if (orders.length === 0) return 'No orders found for this customer.';
    return orders.map((o: OrderWithItems) => {
  ...
  async generate(
    ...
    const customerContext = this.buildContext(chatId);
    ...
    console.log(`[actionService raw] ${raw}`);

    const parsed = extractJson(raw);
    if (!parsed) {
      console.warn('[actionService] could not parse response:', raw);
